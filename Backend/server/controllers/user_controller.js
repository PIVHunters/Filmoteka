...
class UserController{ //Класс для взаимодействия с пользователями
    async registration(req,res,next){ // Функция регистрации пользователя
        ...
        const token = generateJWT(siteUser.id,nickname,email)
    ...
    async login(req,res,next){
        ...
        let user = await pool.query('SELECT id,nickname,password,photo,email FROM public.user WHERE nickname=$1',[nickname])
        ...
        const token = generateJWT(user.rows[0].id,nickname,user.rows[0].photo ,user.rows[0].email)
        return res.json({token})
    }
    async checkUser(req,res){
        const token = generateJWT(req.user.id,req.user.nickname,req.user.photo,req.user.email)
        return res.json({token})
    }
}
